            ...
            public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                if(!queryDocumentSnapshots.isEmpty()){
                    ...
                    for(int i = 0; i < list.size(); i++){
                        SportLocation sl = new SportLocation(list.get(i).getString("name"), list.get(i).getDouble("latitude"), list.get(i).getDouble("longitude"), list.get(i).getString("description"));
                        mMap.addMarker(new MarkerOptions().position(new LatLng(sl.getLatitude(), sl.getLongitude())).snippet(sl.getDescription()).title(sl.getName()));
